...
public:
    ...
    double get_offer() const { return m_offer; }
    double get_bid_quantity() const { return m_bid_quantity; }
    double get_offer_quantity() const { return m_offer_quantity; }
    double get_volume() const { return m_volume; }
    double get_notional() const { return m_notional;}
    double get_limit_up() const { return m_limit_up; }
    double get_limit_down() const { return m_limit_down;}
...
public:
    ...
    double get_value() const { return m_signal;}
    virtual void reset() { m_signal = 0; }
...
{
public:
    ChinaL1DiscreteOtherTranslationFeature(double weight, std::string& trading_instrument,
                                           std::string& reference_instrument, int64_t cutoff_time,
                                           double ratio_mics, bool use_diff, bool corr_negative );
    virtual void reset();
...
protected:
    ...
    {
        return m_corr_negative? (m_use_diff? (mid + other_mid):(mid * other_mid)) :
                                 (m_use_diff?(other_mid - mid):(other_mid/mid));
    }
    double translatedPrice(double other_price) const
    {
        return m_corr_negative ? (m_use_diff? (m_ema.get_value() - other_price):(m_ema.get_value()/other_price)):
            (m_use_diff? (other_price - m_ema.get_value()): (other_price/m_ema.get_value()));

    }

    double translatedDelta(double other_delta, double other_mid) const
    {
        return m_use_diff? other_delta :(m_corr_negative? other_delta * m_ema.get_value()/other_mid/other_mid:
    ...
    double translatedBid(double other_bid, double other_offer) const
    {
        ...
        return m_corr_negative ? translatedPrice(other_offer) : translatedPrice(other_bid);
    ...
    double translatedOffer(double other_bid,double other_offer) const
    {
        ...
        return m_corr_negative ? translatedPrice(other_bid) : translatedPrice(other_offer);
    }

    double signedSignal(double signal) const { return m_corr_negative? (-signal):signal ;}
...
{
public:
    ChinaL1DiscreteOtherTradeImbalanceFeature(double weight, std::string& trading_instrument,
                                           std::string& reference_instrument, int64_t cutoff_time,
                                           double ratio_mics, bool use_diff, bool corr_negative ,double contract_size);
    void reset();
...
{
public:
    ChinaL1DiscreteOtherBookImbalanceFeature(double weight, std::string& trading_instrument,
                                           std::string& reference_instrument, int64_t cutoff_time,
                                           double ratio_mics, bool use_diff, bool corr_negative ,double tick_size);
    void reset();
...
{
public:
    ChinaL1DiscreteOtherBestSinceFeature(double weight, std::string& trading_instrument,
                                           std::string& reference_instrument, int64_t cutoff_time,
                                           double ratio_mics, bool use_diff, bool corr_negative);
    void reset();
...
{
public:
    ChinaL1DiscreteOtherDecayingReturnFeature(double weight, std::string& trading_instrument,
                                           std::string& reference_instrument, int64_t cutoff_time,
                                           double decay_mics);
    void reset();
